...
void Game::movePlayer(Players* player){//moves player
    wait(1000);
    string playerAnswer;//Gets input from player
    int num = diceRoll(), num2 = rand()%2, num3 = rand() %7;//rolls a number 1-6 && creates 2 random numbers for later use
    cout<<player->name<<" rolled a "<<num<<"."<<endl;
    wait(1000);

    //if user rolls a 2 or a 1 give chance to re-roll for a 1, 2, or 3
    if(num != 3 && num != 4 && num != 5 && num != 6)
    {
        cout << "Low roll detected" << endl;
        cout << "Would " << player->name << " like to roll again for a value between 1 and 3?" << endl;

        //if player is human than receive user input
        if(player->human != 0)
        {
            cin >> playerAnswer;
            cin.ignore();
        }
        //else, is bot and randomize choice
        else
        {
            if(num2 == 1)
            {
                playerAnswer = "yes";
                cout << player->name << " decided to re-roll!" << endl;
            }
            else
                playerAnswer = " ";
        }

        //loop through and convert all char in string to lowercase for comparison
        for(int i = 0; i < playerAnswer.size(); i++)
        {
            playerAnswer[i] = tolower(playerAnswer[i]);
        }

        //if yes then re-roll until non-negative/non-zero number occurs
        if(playerAnswer.compare("yes") == 0)
        {
            ...
            {
                num = num * 4;
                cout << player->name << " just hit the jackpot! X4 rolling multiplier!!" << endl;
            }
            cout << player->name << "'s new roll is " << num << "!" << endl;
        }
        else
            cout << "Re-roll declined" << endl;
